...
// [알고리즘]
// 1. n번만큼 반복하며 소수만 배열에 저장하고 해당 배열을 리턴하는 메소드를 선언한다.
// 	1) 길이가 n인 int 배열을 생성한다.
//	2) 소수를 저장할 위치를 나타내는 인덱스 변수를 선언하고 0으로 초기화한다.
// 	3) n번 반복한다.
//		1> 0이상 n미만의 랜덤한 숫자를 얻는다.
//		2> 해당 숫자가 소수인지 판별한다.
//			1] 해당 숫자가 2인 경우(if) 해당 숫자를 배열에 저장하고, 인덱스를 1증가시키고, 2>과정을 종료한다(continue;).
//			2] 해당 숫자를 2부터 해당 숫자 - 1 까지의 숫자로 나누었을 때 나머지가 0 인 경우가 존재하면(else if) 배열의 다음 요소로 넘어간다(break;).
//			이 과정에서 해당 숫자가 0 또는 1인 경우의 판별도 포함한다.
//			3] 나누는 숫자가 해당 숫자 - 1 가 될 경우 2부터 해당 숫자 - 1까지의 숫자로 나누었을 때 나머지가 0 인 경우가 존재하지 않음을 뜻하므로 해당 숫자는 소수이다.
//			따라서 해당 숫자를 배열에 저장하고, 인덱스 변수를 1증가시킨다.
//	4) 1)의 배열에 담긴 소수의 수만큼의 크기를 갖는 int 배열을 생성한다.
//	5) 1)의 배열을 순회하며 4)의 배열에 각 요소를 저장한다.
//	6) 4)의 배열을 리턴한다.

// [예외가 발생할 수 있는 상황]
// 1. 배열의 길이가 양의 정수가 아닌 경우
...
public class Loop4 {
	...
	public static int[] getPrimeNumbers(int n) throws Exception {
		...
		for(int i = 0; i < ARRAY_LENGHT; i++) {
			...
			for(int k = 2; k < randomValue; k++) { //17
				if(randomValue % k == 0) { //
					break;
				}
				if(k == randomValue - 1) {
